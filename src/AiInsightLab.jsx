...
export const AiInsightLab = () => {
    ...
    const fetchLogs = async () => {
        ...
        try {
            ...
            if (error) {
                console.warn('ai_analysis_logs 테이블이 없거나 조회 실패:', error.message);
                setLogs([]);
            } else {
                setLogs(data || []);
            }
    ...
    const filteredLogs = useMemo(() => {
        return logs.filter(row => {
            ...
            if (sourceFilter !== '전체' && row.source_menu !== sourceFilter) return false;
    ...
    }, [logs, confidenceFilter, reviewFilter, sourceFilter, searchValue, dateFilter]);
        ...
        try {
            const { error } = await supabase
                ...
                .update({
                    ...
                    corrected_detail: correctedDetail
                })
                .eq('id', activeRow.id);
    ...
    return (
        <div className="p-6 flex flex-col gap-4 animate-fade-in w-full h-[calc(100vh-64px)] slide-up bg-slate-100">
            ...
            <div className="grid grid-cols-5 gap-4 shrink-0">
                <div onClick={() => { setConfidenceFilter('전체'); setReviewFilter('전체'); setSourceFilter('전체'); }} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col justify-center transition-all hover:shadow-md border-b-4 border-b-blue-400 cursor-pointer">
                    <span className="text-xs font-bold text-blue-500 mb-1">총 누적 AI 분석</span>
                    <span className="text-2xl font-black text-blue-600">{logs.length} <span className="text-sm font-bold text-blue-300 ml-1">건</span></span>
                </div>
                <div onClick={() => { setConfidenceFilter('low'); setReviewFilter('전체'); setSourceFilter('전체'); }} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col justify-center transition-all hover:shadow-md border-b-4 border-b-red-400 cursor-pointer">
                    <span className="text-xs font-bold text-red-500 mb-1">신뢰도 낮음 (요주의)</span>
                    <span className="text-2xl font-black text-red-600">{logs.filter(l => l.ai_confidence === 'low').length} <span className="text-sm font-bold text-red-300 ml-1">건</span></span>
                </div>
                <div onClick={() => { setConfidenceFilter('전체'); setReviewFilter('검토 완료'); setSourceFilter('전체'); }} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col justify-center transition-all hover:shadow-md border-b-4 border-b-green-400 cursor-pointer">
                    <span className="text-xs font-bold text-green-500 mb-1">관리자 보정 완료</span>
                    <span className="text-2xl font-black text-green-600">{logs.filter(l => l.is_reviewed).length} <span className="text-sm font-bold text-green-300 ml-1">건</span></span>
                </div>
                <div onClick={() => { setConfidenceFilter('전체'); setReviewFilter('미검토'); setSourceFilter('전체'); }} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col justify-center transition-all hover:shadow-md border-b-4 border-b-purple-400 cursor-pointer">
                    <span className="text-xs font-bold text-purple-500 mb-1">미검토 건</span>
                    <span className="text-2xl font-black text-purple-600">{logs.filter(l => !l.is_reviewed).length} <span className="text-sm font-bold text-purple-300 ml-1">건</span></span>
                </div>
                <div onClick={() => { setConfidenceFilter('전체'); setReviewFilter('전체'); setSourceFilter('MobileBarcode'); }} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col justify-center transition-all hover:shadow-md border-b-4 border-b-orange-400 cursor-pointer">
            ...
            <div className="w-full bg-white rounded-lg shadow-sm border border-slate-200 px-6 py-3 flex items-center justify-between z-30 shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <label className="text-[11px] font-bold text-gray-600">분석 소스</label>
                        <select value={sourceFilter} onChange={e => setSourceFilter(e.target.value)} className="border border-gray-200 rounded text-xs px-2 py-1.5 focus:outline-none cursor-pointer">
                            <option value="전체">전체</option>
                            <option value="AccidentManagement">📋 사고분석</option>
                            <option value="MobileBarcode">📱 바코드</option>
                        </select>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    <table className="w-full text-left whitespace-nowrap table-fixed min-w-[1200px]">
                        <thead className="bg-slate-50 border-b border-gray-200 text-xs text-slate-500 font-bold sticky top-0 z-10 shadow-sm">
                            <tr>
                                ...
                                {[{ label: '분석 요청 일시', key: 'created_at', w: '160px' }, { label: '메뉴 출처', key: 'source_menu', w: '140px' }, { label: '원본 내용 (클레임 등)', key: 'original_text', w: '300px' }, { label: 'AI 분석 원인', key: null, w: '200px', hasHelp: true }, { label: '신뢰도', key: 'ai_confidence', w: '100px' }, { label: '상태', key: 'is_reviewed', w: '100px' }, { label: '액션', key: null, w: '100px' }].map((col, idx) => (
                                    <th key={idx} className="p-4 text-center select-none" style={{ width: col.w }}>
                                        <div className="flex items-center justify-center gap-1.5">
                                            {col.label}
                                            {col.hasHelp && (
                                                <button onClick={() => setIsCategoryModalOpen(true)} className="text-white bg-slate-300 rounded-full w-[14px] h-[14px] flex items-center justify-center text-[9px] font-black hover:bg-slate-500 transition-colors shadow-sm" title="분류 체계 안내">?</button>
                                            )}
                                        </div>
                                    </th>
                                ))}
                        ...
                        <tbody className="divide-y divide-gray-100 text-[13px] text-gray-700">
                            {isLoading ? (
                                <tr>
                                    <td colSpan="8" className="py-32 text-center align-middle">
                                        <div className="w-8 h-8 border-4 border-blue-100 border-t-letusBlue rounded-full animate-spin mx-auto"></div>
                                        <p className="mt-3 text-gray-500 font-bold text-[13px]">AI 로그를 불러오는 중입니다...</p>
                                    </td>
                                </tr>
                            ) : sortedLogs.length === 0 ? (
                                <tr><td colSpan="8" className="p-20 text-center text-gray-400 font-bold">저장된 AI 로그가 없습니다.</td></tr>
                            ) : sortedLogs.map((row) => (
                                    ...
                                    <td className="p-4 whitespace-normal text-left max-w-[300px] border-l border-gray-50 bg-gray-50/20">
                                        <div className="flex flex-col gap-1.5">
                                            {row.original_text.split('|').map((line, i) => (
                                                <div key={i} className={`text-[11.5px] leading-snug ${i === 0 ? 'text-slate-800 font-bold' : 'text-slate-600'}`}>
                                                    {line.trim()}
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <td className="p-4 text-center">
                                        <div className="flex flex-col items-center gap-0.5">
                                            <div className="flex items-center gap-1.5">
                                                <span className="px-3 py-0.5 rounded-full font-black text-[11px] bg-purple-100 text-purple-700 border border-purple-200 shadow-sm inline-block whitespace-nowrap">
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white rounded-xl shadow-2xl z-10 w-full max-w-lg slide-up border border-gray-100 overflow-hidden flex flex-col">
                        ...
                        <div className="p-5 space-y-5">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-3 bg-purple-50 border border-purple-100 rounded-lg">
                                    <div className="text-[10px] font-bold text-purple-400 mb-1">AI 1차 판별 대분류</div>
                                    <div className="font-black text-purple-700 text-sm">{activeRow.ai_analyzed_cause || '-'}</div>
                                </div>
                                <div className="p-3 bg-purple-50 border border-purple-100 rounded-lg">
                                    <div className="text-[10px] font-bold text-purple-400 mb-1">AI 1차 판별 소분류</div>
                            ...
                            <div className="border-t border-gray-200 pt-5">
                                <div className="flex justify-between items-center mb-3">
                                    <h4 className="text-sm font-black text-gray-800 flex items-center gap-2">
                                        <span>👤</span> 올바른 정답(분류)을 지정해주세요
                                        <button onClick={() => setIsCategoryModalOpen(true)} className="text-white bg-slate-300 rounded-full w-[16px] h-[16px] flex items-center justify-center text-[10px] font-black hover:bg-slate-500 transition-colors shadow-sm ml-1" title="분류 체계 안내">?</button>
                                    </h4>
                                </div>
                        ...
                        <div className="p-4 border-t bg-gray-50 flex justify-between items-center">
                            <div>
                                <button onClick={() => {
                                    if (activeRow.source_menu === 'AccidentManagement') {
                                        window.open(`/accident_list?target_id=${activeRow.target_id}`, '_blank');
                                    } else {
                                        alert('해당 메뉴의 바로가기는 아직 지원되지 않습니다.');
                                    }
