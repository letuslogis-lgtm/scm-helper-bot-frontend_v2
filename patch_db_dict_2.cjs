...
if (content.match(regex)) {
    ...
    // If exact regex fails, do index slicing safely
    let startIndex = content.indexOf('const DB_DICTIONARY = {');
    let endIndex = content.indexOf('};\n\nconst DatabaseDictionary');
    if (endIndex === -1) endIndex = content.indexOf('};\r\n\r\nconst DatabaseDictionary');
    if (startIndex !== -1 && endIndex !== -1) {
        content = content.substring(0, startIndex) + newDict + content.substring(endIndex + 3);
        fs.writeFileSync(p.join(src, 'DatabaseDictionary.jsx'), content);
        console.log('DatabaseDictionary.jsx patched via substring');
    } else {
        console.log('Substring replacement also failed');
    }
}
